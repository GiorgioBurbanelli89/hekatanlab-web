...
document.getElementById('template-select')!.addEventListener('change', (e) => {
  ...
  if (t) { if (timer) clearTimeout(timer); suppressAutorun = true; setEditorText(t.code); suppressAutorun = false; run(); }
});
...
async function loadCpdIndex(): Promise<void> {
  try {
    ...
    if (!res.ok) {
      console.warn('No calcpad-examples/index.json');
      return;
    }
    ...
    console.error('Failed to load Calcpad examples:', err);
  }
}
